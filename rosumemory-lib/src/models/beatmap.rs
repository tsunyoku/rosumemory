...
impl Beatmap {
    ...
    pub unsafe fn from_ptr(pid: usize, ptr: *mut u8) -> Result<Self, ReadMemoryError> {
        ...
        let artist = offset_string!(pid, deref_ptr, 0x1C).unwrap_or(artist_romanised.clone());
        let title_romanised = offset_string!(pid, deref_ptr, 0x24)?;
        let title = offset_string!(pid, deref_ptr, 0x28).unwrap_or(title_romanised.clone());
